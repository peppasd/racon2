...
bool next_round;
void SisdAlignmentEngine::suckdick(
  ...
  for (const auto& it : rank_to_node) {
    ...
    // update H
    for (std::uint64_t j = tid + 1; j < matrix_width; j += tid ) {
      H_row[j] = std::max(
...
Alignment SisdAlignmentEngine::Linear(
  ...
  for(int i = 0; i < THREAD_NUM; i ++){
    ...
    threads[i] = std::thread(&SisdAlignmentEngine::suckdick, this, &rtn, sequence_len, score, i);
  }
