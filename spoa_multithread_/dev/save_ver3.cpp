...
// void SisdAlignmentEngine::parallel_aligment(auto it, int tid){    
//     const auto& char_profile =
//         &(pimpl_->sequence_profile[it->code * matrix_width]);
...
void SisdAlignmentEngine::parallel_aligment( spoa::Graph::Node* const* itt){    
    auto it = *itt;
    const auto& char_profile =
        &(pimpl_->sequence_profile[it->code * matrix_width]);
...
Alignment SisdAlignmentEngine::Linear(
  ...
  for (const auto& it : rank_to_node) {
    // workers->createThread(it);
    // workers->joinThread();

    parallel_aligment(&it);

    std::uint32_t i = pimpl_->node_id_to_rank[it->id] + 1;
    std::int32_t* H_row = &(pimpl_->H[i]);
  ...
  int wdt = matrix_width;
  auto sw_condition = [this, &i, &j, &wdt] () -> bool {
