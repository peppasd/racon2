...
struct SisdAlignmentEngine::threadclass{
  std::thread threads[THREAD_NUM];

  threadclass (){
    for(int i = 0; i < THREAD_NUM; i ++){
      threads[i] = std::thread(&SisdAlignmentEngine::threadclass::printSomething, this, i);
    }
  }
  ...
  // void createThread(auto& it){
  //   for(int i = 0; i < THREAD_NUM; i ++){
  //     threads[i] = std::thread(&SisdAlignmentEngine::parallel_aligment, this, it, i);
  //   }
  // }
  ...
  void printSomething(int threadid){
...
SisdAlignmentEngine::SisdAlignmentEngine(
    ...
    : AlignmentEngine(type, subtype, m, n, g, e, q, c),
      pimpl_(new Implementation()), workers(new threadclass()) {
        workers->joinThread();
        next_round = new bool [THREAD_NUM];
        for(int i = 0; i < THREAD_NUM; i ++){
          next_round[i] = false;
        }
}
...
// void SisdAlignmentEngine::ParaUpdateHscore(&SisdAlignmentEngine)
...
void SisdAlignmentEngine::parallel_aligment(
  ...
  std::mutex m;
  std::condition_variable cv[THREAD_NUM];
  ...
  for (const auto& it : rank_to_node) {
    ...
    // update H
    for (std::uint64_t j = tid + 1; j < matrix_width; j += THREAD_NUM ) {
      H_row[j] = std::max(
    ...
    for (std::uint32_t p = 1; p < it->inedges.size(); ++p) {
      ...
      //   std::unique_lock<std::mutex> lk(m);
         complete_thread.fetch_add(1);
      //   std::cout << "complete_thread num : " << complete_thread  << " tid : " << tid << std::endl;
      // }

    if(complete_thread.load() == THREAD_NUM){
      {
        std::unique_lock<std::mutex> lk(m);
        std::cout << tid << " i'm in" << std::endl;
...
//        std::cout << max_i << " " << max_j << std::endl;
        //next_round.store(true);

        complete_thread.store(0);
        for(int i = 0; i < THREAD_NUM; i ++){
          if(i != tid){
            next_round[i] = true;
            cv[i].notify_one();
      ...
      {
        std::unique_lock<std::mutex> lk(m);
        std::cout << "hello rnd : " << cnt << " tid : " << tid <<  std::endl;

        cv[tid].wait(lk, [this, &tid]{
          this->debug.fetch_add(1);
          return this-next_round[tid];
        });
...
// void SisdAlignmentEngine::suckdick(std::uint32_t sequence_len){
...
Alignment SisdAlignmentEngine::Linear(
  ...
  const auto& rank_to_node = graph.rank_to_node();
  auto rtn = graph.rank_to_node();
  matrix_width = sequence_len + 1;
  max_score = type_ == AlignmentType::kSW ? 0 : kNegativeInfinity;
  max_i = 0;
  max_j = 0;
  ...
  // alignment
  std::thread threads[THREAD_NUM];
  spoa::Graph::Node * it = rank_to_node[0];


  for(int i = 0; i < THREAD_NUM; i ++){
    //threads[i] = std::thread(&SisdAlignmentEngine::parallel_aligment, this, sequence_len, graph, score, i);
    threads[i] = std::thread(&SisdAlignmentEngine::parallel_aligment, this, &rtn, sequence_len, score, i);
  }


  //parallel_aligment(sequence_len, graph, score, i);
  ...
  uint32_t mtw = matrix_width;
  auto sw_condition = [this, &i, &j, &mtw] () -> bool {
