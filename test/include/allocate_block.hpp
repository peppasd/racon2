...
public:
        ...
        , output_mask_(output_mask)
    {
        ...
        // Allocate.
        cudaHostAlloc((void**)&block_data_h_, total_h_, cudaHostAllocDefault);
        block_data_d_ = device_buffer<uint8_t>(total_d_, allocator);
    }

    ~BatchBlock()
    {
        cudaFreeHost(block_data_h_);
    }
    ...
    void get_output_details(OutputDetails** output_details_h_p, OutputDetails** output_details_d_p)
    {
        ...
        offset_d_ += align<int64_t, 8>(output_size_ * sizeof(*output_details_d->consensus));
        if (output_mask_ & OutputType::consensus)
        {
            output_details_d->coverage = reinterpret_cast<decltype(output_details_d->coverage)>(&block_data_d[offset_d_]);
            offset_d_ += align<int64_t, 8>(output_size_ * sizeof(*output_details_d->coverage));
        }
        if (output_mask_ & OutputType::msa)
        {
            ...
            offset_d_ += align<int64_t, 8>(output_size_ * max_sequences_per_poa_ * sizeof(*output_details_d->multiple_sequence_alignments));
        }
    ...
    void get_input_details(InputDetails<SizeT>** input_details_h_p, InputDetails<SizeT>** input_details_d_p)
    {
        ...
        if (output_mask_ & OutputType::msa)
        {
            ...
            offset_d_ += align<int64_t, 8>(max_poas_ * max_sequences_per_poa_ * sizeof(*input_details_d->sequence_begin_nodes_ids));
        }
    ...
    void get_alignment_details(AlignmentDetails<ScoreT, SizeT, TraceT>** alignment_details_d_p)
    {
        ...
        offset_d_ += align<int64_t, 8>(sizeof(*alignment_details_d->alignment_graph) * max_nodes_per_window_ * max_poas_);
        alignment_details_d->alignment_read = reinterpret_cast<decltype(alignment_details_d->alignment_read)>(&block_data_d[offset_d_]);
        offset_d_ += align<int64_t, 8>(sizeof(*alignment_details_d->alignment_read) * max_nodes_per_window_ * max_poas_);
        ...
        if (traceback_alignment_)
        {
            ...
            offset_d_ += align<int64_t, 8>(sizeof(*alignment_details_d->scores) * score_matrix_width_ * score_matrix_height_ * max_poas_);
        }

        if (variable_bands_)
        {
            alignment_details_d->band_starts = reinterpret_cast<decltype(alignment_details_d->band_starts)>(&block_data_d[offset_d_]);
            offset_d_ += align<int64_t, 8>(sizeof(*alignment_details_d->band_starts) * max_nodes_per_window_ * max_poas_);
            alignment_details_d->band_widths = reinterpret_cast<decltype(alignment_details_d->band_widths)>(&block_data_d[offset_d_]);
            offset_d_ += align<int64_t, 8>(sizeof(*alignment_details_d->band_widths) * max_nodes_per_window_ * max_poas_);
            ...
            offset_d_ += align<int64_t, 8>(sizeof(*alignment_details_d->band_head_indices) * max_nodes_per_window_ * max_poas_);
            ...
            offset_d_ += align<int64_t, 8>(sizeof(*alignment_details_d->band_max_indices) * max_nodes_per_window_ * max_poas_);
        }
    ...
    void get_graph_details(GraphDetails<SizeT>** graph_details_d_p, GraphDetails<SizeT>** graph_details_h_p)
    {
        ...
        offset_d_ += align<int64_t, 8>(sizeof(*graph_details_d->nodes) * max_nodes_per_window_ * max_poas_);
        graph_details_d->node_alignments = reinterpret_cast<decltype(graph_details_d->node_alignments)>(&block_data_d[offset_d_]);
        offset_d_ += align<int64_t, 8>(sizeof(*graph_details_d->node_alignments) * max_nodes_per_window_ * CUDAPOA_MAX_NODE_ALIGNMENTS * max_poas_);
        ...
        offset_d_ += align<int64_t, 8>(sizeof(*graph_details_d->node_alignment_count) * max_nodes_per_window_ * max_poas_);
        graph_details_d->incoming_edges = reinterpret_cast<decltype(graph_details_d->incoming_edges)>(&block_data_d[offset_d_]);
        offset_d_ += align<int64_t, 8>(sizeof(*graph_details_d->incoming_edges) * max_nodes_per_window_ * CUDAPOA_MAX_NODE_EDGES * max_poas_);
        graph_details_d->incoming_edge_count = reinterpret_cast<decltype(graph_details_d->incoming_edge_count)>(&block_data_d[offset_d_]);
        offset_d_ += align<int64_t, 8>(sizeof(*graph_details_d->incoming_edge_count) * max_nodes_per_window_ * max_poas_);
        graph_details_d->outgoing_edges = reinterpret_cast<decltype(graph_details_d->outgoing_edges)>(&block_data_d[offset_d_]);
        offset_d_ += align<int64_t, 8>(sizeof(*graph_details_d->outgoing_edges) * max_nodes_per_window_ * CUDAPOA_MAX_NODE_EDGES * max_poas_);
        graph_details_d->outgoing_edge_count = reinterpret_cast<decltype(graph_details_d->outgoing_edge_count)>(&block_data_d[offset_d_]);
        offset_d_ += align<int64_t, 8>(sizeof(*graph_details_d->outgoing_edge_count) * max_nodes_per_window_ * max_poas_);
        ...
        offset_d_ += align<int64_t, 8>(sizeof(*graph_details_d->incoming_edge_weights) * max_nodes_per_window_ * CUDAPOA_MAX_NODE_EDGES * max_poas_);
        graph_details_d->sorted_poa = reinterpret_cast<decltype(graph_details_d->sorted_poa)>(&block_data_d[offset_d_]);
        offset_d_ += align<int64_t, 8>(sizeof(*graph_details_d->sorted_poa) * max_nodes_per_window_ * max_poas_);
        graph_details_d->sorted_poa_node_map = reinterpret_cast<decltype(graph_details_d->sorted_poa_node_map)>(&block_data_d[offset_d_]);
        offset_d_ += align<int64_t, 8>(sizeof(*graph_details_d->sorted_poa_node_map) * max_nodes_per_window_ * max_poas_);
        ...
        offset_d_ += align<int64_t, 8>(sizeof(*graph_details_d->sorted_poa_local_edge_count) * max_nodes_per_window_ * max_poas_);
        if (output_mask_ & OutputType::consensus)
        {
            graph_details_d->consensus_scores = reinterpret_cast<decltype(graph_details_d->consensus_scores)>(&block_data_d[offset_d_]);
            offset_d_ += align<int64_t, 8>(sizeof(*graph_details_d->consensus_scores) * max_nodes_per_window_ * max_poas_);
            ...
            offset_d_ += align<int64_t, 8>(sizeof(*graph_details_d->consensus_predecessors) * max_nodes_per_window_ * max_poas_);
        }

        graph_details_d->node_marks = reinterpret_cast<decltype(graph_details_d->node_marks)>(&block_data_d[offset_d_]);
        offset_d_ += align<int64_t, 8>(sizeof(*graph_details_d->node_marks) * max_nodes_per_window_ * max_poas_);
        graph_details_d->check_aligned_nodes = reinterpret_cast<decltype(graph_details_d->check_aligned_nodes)>(&block_data_d[offset_d_]);
        offset_d_ += align<int64_t, 8>(sizeof(*graph_details_d->check_aligned_nodes) * max_nodes_per_window_ * max_poas_);
        graph_details_d->nodes_to_visit = reinterpret_cast<decltype(graph_details_d->nodes_to_visit)>(&block_data_d[offset_d_]);
        offset_d_ += align<int64_t, 8>(sizeof(*graph_details_d->nodes_to_visit) * max_nodes_per_window_ * max_poas_);
        ...
        offset_d_ += align<int64_t, 8>(sizeof(*graph_details_d->node_coverage_counts) * max_nodes_per_window_ * max_poas_);
        if (output_mask_ & OutputType::msa)
        {
            ...
            offset_d_ += align<int64_t, 8>(sizeof(*graph_details_d->outgoing_edges_coverage) * max_nodes_per_window_ * CUDAPOA_MAX_NODE_EDGES * max_sequences_per_poa_ * max_poas_);
            ...
            offset_d_ += align<int64_t, 8>(sizeof(*graph_details_d->outgoing_edges_coverage_count) * max_nodes_per_window_ * CUDAPOA_MAX_NODE_EDGES * max_poas_);
            graph_details_d->node_id_to_msa_pos = reinterpret_cast<decltype(graph_details_d->node_id_to_msa_pos)>(&block_data_d[offset_d_]);
            offset_d_ += align<int64_t, 8>(sizeof(*graph_details_d->node_id_to_msa_pos) * max_nodes_per_window_ * max_poas_);
        }
